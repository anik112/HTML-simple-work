...
if(!function_exists('getRowCount')){
    function getRowCount($connect,$tableName,$rowName,$colId){

        $getData = $connect->prepare("SELECT COUNT(*) AS ids FROM `$tableName` WHERE $rowName=$colId");
        $getData->execute();
        $count=0;
        $tmp=$getData->fetchAll(PDO::FETCH_OBJ);
        foreach($tmp as $tp){
            $count=$tp->ids;
        }
        return $count;
